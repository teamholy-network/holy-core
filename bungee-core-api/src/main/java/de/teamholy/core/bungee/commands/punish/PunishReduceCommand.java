        ...
        if (args.length == 3) {
            ...
            try {
                ...
                if (isBan) {
                    ...
                    if (newDuration < 0) {
                        sender.sendMessage(Message.PUNISH_PREFIX + "§cThe new ban-time would be less than zero!");
                        return;
                    }
                    ...
                    long newDuration = banTime == -1 ? reduceBy : banTime - reduceBy;
                    if (newDuration < 0) {
                        sender.sendMessage(Message.PUNISH_PREFIX + "§cThe new mute-time would be less than zero!");
                        return;
                    }
