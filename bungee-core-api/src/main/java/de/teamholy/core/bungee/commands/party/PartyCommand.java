...
public class PartyCommand extends Command {
    ...
    private void handlePartyInvite(ProxiedPlayer player, String[] args) {
        ...
        if (!canReceiveInvite(target.getUniqueId(), player.getUniqueId())) {
            player.sendMessage(new TextComponent(PREFIX + " §7" + ((getPlayerColor(target.getUniqueId()) + target.getName()) + " toggled their party invites!")));
            return;
        }
        ...
        if (party.getInvitedPlayers().contains(target.getUniqueId())) {
            player.sendMessage(new TextComponent(PREFIX + ("You already invited " + (getPlayerColor(target.getUniqueId()) + target.getName()) + "§7!")));
            return;
        }
    ...
    private void handlePartyJoin(ProxiedPlayer player, String[] args) {
        ...
        if (party == null) {
            player.sendMessage(new TextComponent(PREFIX + ((getPlayerColor(target.getUniqueId()) + target.getName()) + " §7is not in a party!")));
            return;
        }
        ...
        if (party.isPublic() || partyManager.hasInvite(player, target.getUniqueId())) {
            ...
            player.sendMessage(new TextComponent(PREFIX + ("The player " + (getPlayerColor(target.getUniqueId()) + target.getName()) + " §7did not invite you!")));
        }
    }
    ...
    private void handlePartyDeny(ProxiedPlayer player, String[] args) {
        ...
        player.sendMessage(new TextComponent(PREFIX + ("You declined the party invite from " + (getPlayerColor(target.getUniqueId()) + target.getName()))));
        target.sendMessage(new TextComponent(PREFIX + ((getPlayerColor(player.getUniqueId()) + player.getName()) + " §7has declined your request")));
    }
    ...
    private void handlePartyKick(ProxiedPlayer player, String[] args) {
        ...
        if (party != targetParty) {
            player.sendMessage(new TextComponent(PREFIX + ((getPlayerColor(target.getUniqueId()) + target.getName()) + " §7is not in your party!")));
            return;
        }

        String targetName = getPlayerColor(target.getUniqueId()) + target.getName();
        broadcastToParty(party, partyPlayer ->
            partyPlayer.sendMessage(new TextComponent("§5Party §8× " + ((targetName) + " §7was kicked out of the party")))
        );
    ...
    private void handleRemoveInvite(ProxiedPlayer player, String[] args) {
        ...
        if (!party.getInvitedPlayers().contains(target.getUniqueId())) {
            player.sendMessage(new TextComponent(PREFIX + ((getPlayerColor(target.getUniqueId()) + target.getName()) + " §7is not invited!")));
            return;
        }

        party.getInvitedPlayers().remove(target.getUniqueId());
        player.sendMessage(new TextComponent(PREFIX + ("The invite of " + (getPlayerColor(target.getUniqueId()) + target.getName()) + " §7was removed")));
    }
    ...
    private void joinParty(ProxiedPlayer player, Party party, ProxiedPlayer partyLeader) {
        ...
        broadcastToParty(party, partyPlayer -> {
            if (!partyPlayer.getUniqueId().equals(player.getUniqueId())) {
                partyPlayer.sendMessage(new TextComponent("§5Party §8× " + ((playerName) + " §7joined the party")));
            }
        });
    }
    ...
    private void sendInviteMessage(ProxiedPlayer sender, ProxiedPlayer target) {
        ...
        target.sendMessage(new TextComponent("§7" + ("You got an partyinvite from " + (getPlayerColor(sender.getUniqueId()) + sender.getName()))));
        target.sendMessage(
