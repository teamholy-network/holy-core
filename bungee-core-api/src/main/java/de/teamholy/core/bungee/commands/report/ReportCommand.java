...
public class ReportCommand extends Command {
    ...
    public void execute(CommandSender commandSender, String[] args) {
        ...
        if (args.length == 2) {
            ...
            if (!isValidReason) {
                player.sendMessage(prefix + BungeeUtil.format("The reason {} is not a valid reason!", "§e" + args[1] + "§7"));
                return;
            }
            ...
            player.sendMessage(prefix + BungeeUtil.format("You've reported {} for {}", reported + "§7", "§e" + report.getReason()));
            ...
            ProxyServer.getInstance().getPlayers().forEach(proxiedPlayer -> {
                ...
                proxiedPlayer.sendMessage(prefix + BungeeUtil.format("The player {} §7has reported {} §7for {}", reporter, reported, "§e" + report.getReason()) + " §8(§e" + finalTarget.getServer().getInfo().getName() + "§8) " + (finalIsNicked ? "§8(§5§lNICKED§8)" : ""));
                if (finalChatLog != null)
                    proxiedPlayer.sendMessage(prefix + "Chatlog -> https://teamholy.de/chatlog/" + finalChatLog.getChatLogId());
                TextComponent message = new TextComponent(prefix + "§a§l" + "Accept report");
                message.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/reports accept " + finalTarget.getName()));
                message.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§7" + BungeeUtil.format("Accept the report of {}", reported))));
