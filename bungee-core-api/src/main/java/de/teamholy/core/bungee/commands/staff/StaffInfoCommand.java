...
public class StaffInfoCommand extends Command {
    ...
    private String formatStaffProfile(StaffProfile profile, String username, UUID author) {
        ...
        builder.append(((Message.STAFF_INFO_PREFIX) + "Here are the staff stats of §2" + (username) + "§7:"));
        builder.append("\n" + ("§7Banned players §8» §2" + (String.valueOf(profile.getBanProfileList().size()))));
        builder.append("\n" + ("§7Muted players §8» §2" + (String.valueOf(profile.getMuteProfileList().size()))));
        builder.append("\n" + ("§7Finished reports §8» §2" + (String.valueOf(profile.getReportList().size()))));
    ...
    private String formatTimeStaffProfile(StaffProfile profile, String username, int days, UUID author) {
        ...
        StringBuilder builder = new StringBuilder();
        builder.append(((Message.STAFF_INFO_PREFIX) + "Here are the staff stats of §2" + (username) + " §7§o(last " + (String.valueOf(days)) + "d)§r§8"));
        builder.append("\n" + ("§7Banned players §8» §2" + (String.valueOf(profile.getBanProfileList()
            .stream()
            .filter(banProfile -> banProfile.getCreateDate() >= (System.currentTimeMillis() - timemillis))
            .count()))));
        builder.append("\n" + ("§7Muted players §8» §2" + (String.valueOf(profile.getMuteProfileList()
            .stream()
            .filter(muteProfile -> muteProfile.getCreateDate() >= (System.currentTimeMillis() - timemillis))
            .count()))));
        builder.append("\n" + ("§7Finished reports §8» §2" + (String.valueOf(profile.getReportList()
            .stream()
