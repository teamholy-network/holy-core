...
public class LinkManager {
    ...
    public void relinkPlayer(ProxiedPlayer player) {
        ...
        if (statsProfile == null) {
            player.sendMessage("§6Web §8× §7" + "You are not linked!");
            return;
        }

        if (System.currentTimeMillis() - statsProfile.getPlayerProfileLastTimeLinked() < 60 * 60 * 1000) {
            player.sendMessage("§6Web §8× §7" + BungeeUtil.format("Please wait around {} minutes before you can relink your account!", "§e" + helpers.getRemainingTime(statsProfile.getPlayerProfileLastTimeLinked(), 60 * 60 * 1000) + "§7"));
            return;
        }
    ...
    public void sendLinkMessageToPlayer(ProxiedPlayer player, String linkCode) {
        TextComponent message = new TextComponent("§6Web §8× §a" + "Your Link has been generated." + " §7(" + "Click me" + "!)");
        message.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://teamholy.de/link/" + linkCode));
        message.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§6" + "Click").create()));
    ...
    public void sendConfirmRelinkOrLinkMessageToPlayer(ProxiedPlayer player) {
        ...
        if (playerLinked(player)) {
            message.addExtra("§6Web §8× §7" + "You are already linked!");
            message.addExtra("\n");
        }
        message.addExtra("§6Web §8× §7" + "We have noticed that you didn't login for a day!" + "§7 (§6" + "Click me)" + "!§7) " + "if you wish to relink.");
