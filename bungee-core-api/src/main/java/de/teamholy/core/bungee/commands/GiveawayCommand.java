...
public class GiveawayCommand extends Command implements Listener {
    ...
    private void startGiveaway(Integer max, ProxiedPlayer player, String price) {
        ...
        chat = false;
        TextComponent message = new TextComponent("§c§kwsd §f"+"Hover me to see the number"+" §c§kwsd");
        ...
        for (ProxiedPlayer proxiedPlayer : ProxyServer.getInstance().getPlayers()) {
            ...
            proxiedPlayer.sendMessage(prefix + "§l" + BungeeUtil.format("The number is between {} - {}", "§c§l0", "§c§l0"+max));
            proxiedPlayer.sendMessage(prefix + "§l" + "Price" + ": §a§n" + price.replace("&", "§"));
            proxiedPlayer.sendMessage(prefix + "§l" + "Try to guess the number in the chat!");
            proxiedPlayer.sendMessage(prefix + "§l"+ "Good luck, the chat is enabled in 10 seconds!");
            proxiedPlayer.sendMessage("§8§m--------------------------");
        }
        ProxyServer.getInstance().getScheduler().schedule(BungeeCore.getInstance(), () -> {
            chat = true;
            ProxyServer.getInstance().getPlayers().forEach(players -> players.sendMessage(prefix + "§f"+"The chat is now enabled!"));
        }, 10, TimeUnit.SECONDS);
    }
        ...
        if (!chat && !player.hasPermission("*")) {
            ...
            if (event.getMessage().equalsIgnoreCase(String.valueOf(winnerNumber))) {
                ...
                ProxyServer.getInstance().getPlayers().forEach(players -> {
                    players.sendMessage(prefix + "§6§l"+BungeeUtil.format("The Player {} §6§lhas won!", BungeeCore.getInstance().getPlayerColor(player.getUniqueId()) + player.getName()));
                    players.sendMessage(prefix + "§6§l"+"Number"+"§l§8: §c§l" + winnerNumber);
                    players.sendMessage(prefix + "§7§l"+"The chat is enabled in 10 seconds!");
                });
                ProxyServer.getInstance().getScheduler().schedule(BungeeCore.getInstance(), () -> {
                    ProxyServer.getInstance().getPluginManager().unregisterListener(this);
                    ProxyServer.getInstance().getPlayers().forEach(players -> players.sendMessage(prefix + "§6"+"The chat is now enabled!"));
                }, 10, TimeUnit.SECONDS);
            }
        }
