...
public class KickCommand extends SenderCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        target.disconnect("§c" + ("You have been kicked from the " + ("TeamHoly.DE") + " network!") + " \n§7" + "Reason" + " §8» §e" + reason);
        ...
        for (ProxiedPlayer staffNotifyPlayer : BungeeCore.getInstance().getBungeePlayerManager().getStaffNotifyPlayers()) {
            staffNotifyPlayer.sendMessage(prefix + ((kicked + "§7") + " got kicked by " + (name + "§7") + " for " + ("§c" + reason + "§7")));
        }
    }
}
