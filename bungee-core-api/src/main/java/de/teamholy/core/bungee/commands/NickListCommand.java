...
public class NickListCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (onlineNickMap.isEmpty()) {
            sender.sendMessage("§c" + "There are currently no nicked players");
        } else {
            sender.sendMessage(BungeeUtil.format("§7There are currently §e{} nicked §7users", String.valueOf(onlineNickMap.size())));
            sender.sendMessage("");
