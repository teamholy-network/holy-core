            ...
            if (args.length == 1) {
                ...
                if (args[0].equalsIgnoreCase("delete")) {
                    ...
                    if (promoteProfile.getClanRank() == ClanRank.LEADER) {
                        sender.sendMessage(Message.CLAN_PREFIX + "§cThe player is already a " + promoteProfile.getClanRank().getFancy() + "§c!");
                        return;
                    }
                ...
                if (args[0].equalsIgnoreCase("changecolor")) {
                    ...
                    if (clan == null) {
                        sender.sendMessage(Message.CLAN_PREFIX + "§cThis clan does not exists!");
                        return;
                    }

                    if (clan.getMembers().size() >= 50) {
                        sender.sendMessage(Message.CLAN_PREFIX + "§cThe clan reached the limit of 50 members!");
                        return;
                    }

                    sender.sendMessage(Message.CLAN_PREFIX + "§aThe player is now in the " + clan.getColor() + clan.getName() + " §aclan!");
                    ...
                    BungeeCore.getAPI().getClanManager().updateClan(clan);
                    BungeeCore.getInstance().getBungeePlayerManager().sendClanMessage(clan, Message.CLAN_PREFIX + BungeeCore.getAPI().getCloudManager().getColor(uuid) + "§7 joined the clan.");
