...
public class LookupCommand extends SenderCommand {
    ...
    private void handleRankHistory(ProxiedPlayer player, PlayerProfile playerProfile,
        ...
        BungeeCore.getAPI().getRankManager().getRanks(playerProfile.getPlayerId())
                .whenCompleteAsync((ranks, throwable) -> {
                    if (throwable != null) {
                    ...
                    ranks.get().forEach(rank -> {
                        String timeInfo = formatRankTime(
                                rank.getExpiry() == null ? 0 : rank.getExpiry().toEpochMilli());
                        player.sendMessage(new TextComponent(" §8- "
                                + BungeeCore.getAPI().getRankManager().getRankDisplay(
                                        BungeeCore.getAPI().getRankManager().getGroup(rank.getGroupName()))
                                + " §8┃ " + timeInfo));
                    });
    ...
    private void displayOnlineStatus(ProxiedPlayer player, PlayerProfile playerProfile) {
        ...
        if (playerProfile.isOnline()) {
            onlineComp.addExtra(new ChatAction()
                    ...
                    .hover("§7" + ("Click to jump on " + (playerProfile.getServerName())))
                    .component());
        } else {
    ...
    private void displayRankInfo(ProxiedPlayer player, PlayerProfile playerProfile) {
        BungeeCore.getAPI().getRankManager().getPrimaryGroup(playerProfile.getPlayerId())
                .whenCompleteAsync((ranks, throwable) -> {
                    if (throwable != null) {
                    ...
                    InheritanceNode inheritanceNode = ranks.get();
                    String timeInfo = formatRankTime(
                            inheritanceNode.getExpiry() == null ? 0 : inheritanceNode.getExpiry().toEpochMilli());

                    TextComponent rankComp = new TextComponent("§7Highest Rank §8» ");

                    rankComp.addExtra(new ChatAction()
                            .text(BungeeCore.getAPI().getRankManager()
                                    .getRankDisplay(BungeeCore.getAPI().getRankManager()
                                            .getGroup(inheritanceNode.getGroupName()))
                                    + " §8┃ " + timeInfo)
    ...
    private void displayBanHistory(ProxiedPlayer player, PunishHistoryProfile historyProfile,
        ...
        historyProfile.getBanProfileMap().values().stream()
                ...
                .limit(10).skip((page - 1) * 10L).map(banProfile -> getHistoryEntry(player, banProfile))
    ...
    public void printPunish(ProxiedPlayer player, String name, String type, String author, String reason, String until,
            String evidence) {
