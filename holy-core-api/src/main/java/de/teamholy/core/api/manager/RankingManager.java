...
public class RankingManager {
    ...
    CoreAPI coreAPI;
    HashMap<String, RScoredSortedSet> sortedSetHashMap;

    public RankingManager(CoreAPI coreAPI) {
        this.coreAPI = coreAPI;
        this.sortedSetHashMap = new HashMap<>();

        for (Gamemodes gamemodes : Gamemodes.values()) {
            for (StatsType statsType : StatsType.values()) {
                RScoredSortedSet scoredSortedSet = coreAPI.getRedissonManager()
                    .getRedissonClient()
                    .getScoredSortedSet(gamemodes.toString() + "_" + statsType.toString());
    ...
    public UUID getUUIDFromRank(Gamemodes gamemodes, StatsType statsType, int rank) {
        try {
            return (UUID) sortedSetHashMap.get(gamemodes + "_" + statsType)
                .entryRangeReversed((rank + 1), (rank + 1))
                .stream()
                .toList()
                .get(0);
