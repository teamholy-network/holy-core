...
public class AccountInfoCommand extends Command implements TabExecutor {
    ...
    public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (!sender.hasPermission("holylogin.accountinfo")) return sug;

            ProxyServer.getInstance().getPlayers().stream()
                .filter(proxiedPlayer -> proxiedPlayer.getName().startsWith(complete))
                .forEach(proxiedPlayer -> sug.add(proxiedPlayer.getName()));
        }
        return sug;
    }
}
